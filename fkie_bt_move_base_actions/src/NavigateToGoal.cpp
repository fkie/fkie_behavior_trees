  ...
  : BT::CoroActionNode(name, config)
{
  ...
  sub_notify = public_node.subscribe(topic_notify_result, 1, &NavigateToGoal::callbackNavigationResult, this);
...
BT::NodeStatus NavigateToGoal::tick()
{
  ...
  if (!is_initialized)
  {
    BT_ROS_ERROR("NavigateToGoal is not initialized");
    return BT::NodeStatus::FAILURE;
  }

  BT_ROS_DEBUG_STREAM("NavigateToGoal::tick()");
  ...
  BT_ROS_DEBUG_STREAM("NavigateToGoal: Navigation finished");
...
void NavigateToGoal::callbackNavigationResult(const fkie_bt_move_base_actions::NavigationResult& msg)
{
  new_message_received = true;
  last_msg = msg;
